...
class Proxy_Class:
    ...
    def change_ip(self,proxy_refresh_limit=60, only_check_inet=False):
        s = self.get_session()
        ...
        for cycle_index in range(10):
            ...
            try:
                ...
                if self.logging:
                    logger.success(f'{self.logging}Новый айпи - {response.json()["origin"]}')
    ...
    def check_connection(self):
        s = self.get_session()
        for i in range(3):
            try: return s.get('https://httpbin.org/ip').json()['origin']
            except:sleep(5)
        return None
    def get_session(self):
        ...
        s = requests.Session()
        s.proxies ={'https':f'http://{self.url_proxy}', 'http':f'http://{self.url_proxy}'}
        s.auth = auth
        return s
